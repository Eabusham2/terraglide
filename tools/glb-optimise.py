...
for mesh in J['meshes']:
    for prim in mesh['primitives']:
        ...
        #
        # The whole flat slice goes, sole and all.
        #
        # Keeping the sole was tried twice and fails on paint, not on shape.
        # Grouping the flat, low triangles by what they are joined to does
        # separate them — a slab of ten thousand reaching half a body-length
        # out, and four patches of twenty-odd under the boots that really are
        # the generator's own sole. But those patches are welded into the
        # floor, so they carry the *floor's* texture coordinates, and keeping
        # them puts four bright blue-grey wedges of ground into the toe of
        # each boot. Better a sole that is one flat colour taken off the boot
        # than four pieces of lawn set into it.
        keep = []
        for t in range(0, len(idx), 3):
