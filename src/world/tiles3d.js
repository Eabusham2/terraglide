 ...
 */
const DETAIL = {
  low: { sse: 48, loaded: 90, active: 12, tilesets: 6 },
  medium: { sse: 32, loaded: 160, active: 20, tilesets: 8 },
  high: { sse: 24, loaded: 220, active: 28, tilesets: 10 },
  ultra: { sse: 16, loaded: 340, active: 40, tilesets: 14 },
};
/*
 * Why those numbers went up, and an admission.
 *
 * They were four to eight, and they were never a real limit: the slot was
 * released fifty milliseconds after a request *started*, so the pipe was
 * effectively unbounded and full of duplicate requests for tiles already on
 * their way. Fixing that made the number mean what it says — and turned an
 * unbounded pipe into a strictly-six one, which on a phone is slower than what
 * it replaced. That is my doing and this is the correction.
 *
 * Six is the browser's per-host limit for HTTP/1.1. Both of these servers speak
 * HTTP/2, where requests are multiplexed over one connection and a few dozen in
 * flight is ordinary rather than aggressive. With duplicates actually
 * suppressed there is nothing to gain from holding the queue back.
 */
/**
...
/**
 * How deep the walk may go before it is treated as runaway.
 *
 * This counts tiles and the hops between the tilesets they are spread across,
 * both. Google's photorealistic tree reaches street level well past twenty-four
 * of those, which is what the cap used to be, so it was being hit in ordinary
 * play and every tile it stopped at was reported to its parent as covered.
 * Sixty-four is still a guard against a cyclic tileset and is far past anything
 * a real tree uses.
 */
const MAX_DEPTH = 64;
/**
...
const DATUM_BIN_M = 2;
/**
 * How dense a cluster has to be, against the densest, to be treated as the
 * ground rather than as something under it.
 *
 * It was two fifths, and that let a pile of undersides seen through holes in
 * the shells outvote the street by being lower: measured live, the answer
 * drifted 32.4, 32.8, 38.6 as more of the city arrived, against a known 32.8.
 * At nine tenths only a cluster genuinely as dense as the street can take the
 * tie-break, which is the case the low-preference is actually for: roofs sit
 * above the street and vary in height, so they never form one that dense.
 */
const DATUM_CLUSTER_SHARE = 0.9;
...
/**
 * How many measurements in a row have to agree before the ground you stand on
 * is allowed to follow the tiles.
 *
 * The lift is worked out from what has loaded, and a city streams in. Traced
 * live at Stevenson Street, every three seconds, while 238 tiles arrived:
 *
 *   0, 19.7 ×5, 41.9, 19.7, 42, 52, 61.3, 25.7, 25.7, 42.3, 52, 65.7,
 *   52.5, 32, 32, then 32.5 for the remaining three minutes
 *
 * Forty-six metres of swing before it settles, and the settled answer agrees
 * with the +32.9 measured for G23. That was harmless while the datum only
 * moved the *drawing*. It is not harmless now the player stands on these
 * surfaces: 65.7 against a true 32.5 would have put you thirty-three metres
 * above the street, in the air.
 *
 * So the floor waits for the answer to hold still, and this is how long.
 * Counting the run of agreeing measurements rather than eyeballing the trace:
 * across the whole noisy stretch no wrong value ever got past three in a row —
 * 19.7 reached three, 25.9 and 42.3 reached two, the rest one or none — while
 * the settled 32.3 climbed 1, 3, 5, 7, 8, 10, 12, 14, 16 and kept going.
 * Fifteen sits five times clear of the highest wrong run and is reached about
 * forty-five seconds after the city stops moving.
 *
 * A number taken from that trace rather than derived, and conservative on
 * purpose: being wrong here throws the player into the sky, and being slow
 * here only means standing on the height field a little longer, which is
 * exactly what the game did before any of this.
 *
 * Two things it is not. It is not a gate on the *walls* — a wall is vertical,
 * and where it stands in plan does not depend on the datum, which is why you
 * stop walking through buildings immediately. And it is not measured by how
 * much of the city has loaded: that was tried, counting the share of the
 * datum's own sample points that found any surface, and it read 1.00 from the
 * first measurement to the last, because the coarse tiles cover the ground
 * long before the detail that makes the answer right. It does not discriminate
 * and it is not in the code.
 */
const DATUM_STEADY = 15;
...
/**
 * Make a fetched tileset's content URIs absolute against its own URL.
 *
 * Walks the tile tree once, when it lands, so nothing downstream has to know
 * which tileset a tile came from.
 */
export function rebase(tile, base) {
  if (!tile) return;
  ...
  if (uri) {
    try {
      ...
      if (content.uri !== undefined) content.uri = absolute;
      else content.url = absolute;
    } catch { /* leave it alone rather than replace it with something worse */ }
  }
  for (const child of tile.children ?? []) rebase(child, base);
}

/**
 * Say what actually went wrong, in the provider's own words where it has any.
 *
 * Both APIs answer failures with a JSON body containing a plain-English reason.
 * Throwing away the reason and reporting the status code turned "the Map Tiles
 * API has not been used in project 12345 before or it is disabled" — which
 * names the project and the fix — into "root 400".
 */
export async function explain(response) {
  ...
  if (response.status === 401 || response.status === 403) return 'that key was refused';
  return `the tileset server answered ${response.status}`;
}
...
export class Tiles3D {
  constructor({ scene, frame, camera, renderer }) {
    ...
    this._datumAt = 0;
    /**
     * How many datum measurements in a row have agreed. See DATUM_STEADY: the
     * floor follows the tiles only once the lift has stopped moving, and an
     * undefined counter here would open that gate on the first frame.
     */
    this.datumSteady = 0;
  ...
  async start() {
    ...
    try {
        ...
        if (!endpoint.ok) {
          throw new Error(
            endpoint.status === 401 || endpoint.status === 403
              ...
              : `Cesium ion: ${await explain(endpoint)}`,
          );
        }
      ...
      if (!response.ok) {
        // Google answers a bad key with 400, not 401 or 403, so every real
        // key problem came out as "root 400" — a number that tells you
        // nothing and points nowhere. It also says exactly what is wrong in
        // the body, and the commonest answer by far is that the Map Tiles API
        // has never been enabled on that project, which is a thing you fix in
        // the Cloud console and could not possibly guess from a status code.
  ...
  measureDatum(now) {
    ...
    /*
      Take the densest cluster, not the lowest dense one.

      "Lowest dense" was wrong and it drifted: measured live at Market Street it
      went 32.4, 32.8, 38.6 as the city filled in, against a known 32.8. What
      arrives late is not more street — it is more of the things below it. A ray
      down a column can pass through a hole in a shell and hit an underside, and
      once enough of those pile into one bin they clear a two-fifths share of
      the densest and win the tie-break by being lower.

      The street is the one surface present in every column, so it is the
      densest bin, full stop. Undersides are sparse by nature: they need a hole
      above them. Only among bins genuinely as dense as each other is being
      lower still the better answer, since roofs sit above the street and vary
      in height while the street does not.
    */
    let ground = null;
    ...
    const lift = -measured;
    if (Math.abs(lift - this.datum) < 0.5) {
      // Agreeing with itself is the signal the floor waits for.
      this.datumSteady++;
      return;
    }
    this.datumSteady = 0;
  ...
  floorAt(x, z, ceiling, drop = FLOOR_DROP_M) {
    // Not until the lift these surfaces sit on has stopped moving: standing on
    // a surface whose height is still being worked out is standing on a moving
    // floor. See DATUM_STEADY.
    if (this.datumSteady < DATUM_STEADY) return null;
  ...
  traverse(tile, parentTransform, cameraEcef, screenHeight, fov, depth) {
    // The return value means "everything this subtree wants to draw is drawn",
    // and a REPLACE parent stops drawing itself the moment its children all say
    // yes. So every early exit that answers `true` is a parent dropped over
    // ground nobody drew — a hole exactly the shape of one tile.
    //
    // There were four of them: past the depth cap, an unreadable bounding
    // volume, past the render distance, and no content. The first three are
    // "I gave up here", which is the opposite of "I covered this", and they are
    // why whole blocks of the city are missing with a flat plate underneath.
    // They answer `false` now, so the coarse parent keeps drawing — which is
    // the rule the rest of this function already follows.
    if (!tile) return true;
    // And the cap itself was far too shallow. Depth counts tiles *and* the hops
    // between the tilesets they are spread across, and Google's tree at street
    // level is deeper than twenty-four of those combined, so the cap was being
    // hit in ordinary play rather than as the runaway guard it is meant to be.
    ...
    // Beyond the render distance there is no point even considering it — but
    // saying so is not the same as covering it, and a parent that straddles the
    // edge used to be dropped over the half that was never drawn.
  ...
  requestTileset(uri) {
    ...
    fetch(this.absolute(uri), { headers: this.headers() })
      ...
      .then((json) => {
        // A child tileset's contents are relative to *that* tileset, not to the
        // root. Resolving them against the root works for Google, whose URIs are
        // absolute paths, and is wrong for anything that uses relative ones —
        // including ion's own OSM Buildings, which this offers in the settings.
        // Fixing it here means everything downstream sees absolute URIs.
        rebase(json.root, this.absolute(uri));
